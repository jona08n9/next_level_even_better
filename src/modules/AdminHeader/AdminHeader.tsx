...
export const AdminHeader = ({ pageList }: HeaderProps) => {
  ...
  return (
    <>
      <div className={` bg-primaryCol w-screen h-screen fixed top-0 transition-all md:hidden overflow-scroll pb-6 z-[11]  ${isOpen ? 'right-0' : 'right-full'} flex flex-col pt-28`}>
        <ul className='flex mx-8 flex-col gap-8  '>
          {pageList.map((pages) => {
            return (
              <>
                <li key={Math.floor(Math.random() * 1000).toString()}>
                  {pages.page.subPages ? (
                    <div>
                      <a href={pages.page.href} className='font-bold uppercase text-2xl' key={Math.floor(Math.random() * 1000).toString()}>
                      ...
                      <div className='pl-8 flex flex-col gap-2 mt-3 '>
                        {pages.page.subPages.map((subpage) => {
                          return (
                            <>
                              <a href={subpage.href} className='' key={Math.floor(Math.random() * 1000).toString()}>
                                {subpage.pageTitle}
                              </a>
                            </>
                          );
                    ...
                    <a href={pages.page.href} className='border-2 uppercase text-2xl font-bold border-transparent hover:border-b-accentCol ' key={Math.floor(Math.random() * 1000).toString()}>
                      {pages.page.pageTitle}
                    </a>
                  )}
                </li>
      ...
      <nav className='fixed top-2 px-6 md:px-12 lg:px-20 z-20 w-full '>
        <div className={`my-3 backdrop-blur-sm ${isOpen ? 'bg-primaryCol' : 'bg-contrastCol/70'} flex justify-between   items-center px-4 gap-6 rounded-sm md:py-2 md:bg-contrastCol/70 max-w-screen-xl xl:mx-auto`}>
                ...
                return (
                  <>
                    {pages.page.subPages ? (
                      ...
                      <a href={pages.page.href} className='border-2 border-transparent hover:border-b-accentCol' key={Math.floor(Math.random() * 1000).toString()}>
                        {pages.page.pageTitle}
                      </a>
                    )}
                  </>
          ...
          <Button className='hidden  my-1 md:!flex' onClick={signUserOut}>
            Log ud
          </Button>
        </div>
      </nav>
